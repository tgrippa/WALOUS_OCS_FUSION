...
def ComputeERPfromCsv(in_path, delimiter=',', erp_name="ERP", start_index=1):   # "start_index" should contain the index where probabilities starts, "stop_index" where it stops.
    # Open files and create reader
    infile = open(in_path, 'r')
    creader = csv.reader(infile, delimiter=delimiter)

    # Csv header
    out_header = creader.next() #Get the header (first line) of input csv
    ...
    cwriter = csv.writer(outfile, delimiter=delimiter)
    cwriter.writerows(out_content)
